...
# states to incidentsのブラックリストを削除したもの
# ライブラリ読み込み
library(bnlearn)

#データ読み込み,処理
product <- read.csv("~/rscripts/data/250124_4/data_products_20250124.csv")
product <- as.data.frame(lapply(product, factor))

states <- read.csv("~/rscripts/data/250124_4/data_states_20250124.csv")
states <- states$states

incidents <- read.csv("~/rscripts/data/250124_4/data_incidents_20250124.csv")
incidents <- incidents$incidents

causes <- read.csv("~/rscripts/data/250124_4/data_causes_20250124.csv")
...
# DOT言語のファイルを出力
write.dot("~/rscripts/output/data_nobl_s2i_250124_4.dot", fitted)
